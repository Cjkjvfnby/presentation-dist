...
..
  При инсталляции можно настраивать много чего интересного.

  По умолчанию пип ищет пакеты в репозитории PyPI, но это можно поменять.
  Для этого используется стандартная практика задачи дополнительных настроек.
  Сначала пип смотрит на аргументы командной строки, потом на переменные окружения, потом на свой файл конфигурации,
  и если нигде на нашел, использует значение по умолчанию.

  Пупи содержит информацию о версиях пакетов, этим можно пользоваться при установке библиотеки.
  ...
  Я выучил это через сложный путь, вечерний релиз затянулся на три часа,
  просто потому, что версия одной из библиотек была не зафиксирована, и ей обновление нарушило работу программы.

  Пип позволяет указывать сразу несколько библиотек через пробел. Если у библиотек есть общие зависимости, то они будут скачаны один раз.
  ...
  Это один из способ хранить список зависимостей.
  ...
  Например продукт состоит из разных сервисов, у каждого из которых свои требования, но на машине разработчика хочется поставить всё стразу.
