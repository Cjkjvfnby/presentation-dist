...
..
  ...
  Обычно такие требования появляются когда есть пользователи библиотеки в других отделах.
  ...
  Можно добавить скрипты которые будет выполнятся в системе как программы. пример такого скрипта это pip.

  Публичную библиотеку можно опубликовать на PyPI. Это упростит её установку.
  Для публикации на ПуПи требуется более полное заполнение данных и наличие аккаунта. Он бесплатный.
  Подробнее рассматривать инструкции не имеет смысла, это достаточно редкий процесс.

  Стоит упомянуть про форматы в которых хранятся пакеты это более новый Wheel и устаревший Egg.
  Пип поддеживает оба формата, я на практике не сталкивался с ситуацией когда мне бы пришлось узнавать о формате более подробно.
